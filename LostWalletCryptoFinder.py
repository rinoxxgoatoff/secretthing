...
# Affiche un wallet trouvé toutes les 10 secondes
...
class CryptoUI(tk.Tk):
    def __init__(self):
        ...
        self.crypto_codes = []
        self._build_ui()
        self.counter_label.config(text=f"Itérations : {self.iterations}")
        self.found_box.delete(0, tk.END)
        sol_addr = generate_real_address("SOL")
        self.found_box.insert(tk.END, f"FOUND ► SOL • $154 • {sol_addr}")
    ...
    def _build_ui(self):
        ...
        self.counter_label = tk.Label(panel, text="Itérations : 0", bg=CARD, fg=SUBTEXT, font=("Segoe UI", 10))
    ...
    def start(self):
        ...
        self._feed_tick()
        # Le premier résultat tombera exactement dans 1 heure
        self.after(FOUND_INTERVAL_MS, self._found_tick)

    def stop(self):
        self.running = False
        self.status_label.config(text="arrêté", fg=SUBTEXT)
    ...
    def _feed_tick(self):
        ...
        if selected:
            ...
            self.feed_box.insert(tk.END, f"{coin.ljust(4)} • {addr}")
    ...
    def _found_tick(self):
        ...
        if selected:
            coin = random.choice(selected)
            addr = generate_real_address(coin)
            self.found_box.insert(tk.END, f"FOUND ► {coin} • ${random.randint(40,900)} • {addr}")
            self.found_box.see(tk.END)
